...
graph_v_of_v_idealID graph_hash_of_mixed_weighted_to_ideal_graph_of_CT(graph_hash_of_mixed_weighted& input_graph, vector<vector<midnode>>& midnode_graph, int max_N_ID)
{
		...
		if (search != input_graph.hash_of_hashs.end())
		{
			for (auto it2 = search->second.begin(); it2 != search->second.end(); it2++)
			{
				...
				if (i < j)
				{
					...
					pair<int, double> p1(j, ec);
					output_graph[i].push_back(p1);
					struct midnode md1 = { j,i };
					midnode_graph[i].push_back(md1);
					pair<int, double> p2(i, ec);
					output_graph[j].push_back(p2);
					struct midnode md2 = { i,j };
					midnode_graph[j].push_back(md2);
...
void substitute(graph_v_of_v_idealID& ideal_graph, int v_x, int v_y, double ec, vector<vector<struct midnode>>& midnode_graph, int l, int r)
{
	bool flag = 0; // check if the edge already exists
	int v_adj_size = ideal_graph[v_x].size();
	for (int i = 0; i < v_adj_size; i++)
		if (ideal_graph[v_x][i].first == v_y) // the edge already exists； since ideal_graph is not sorted here, searching edge is very slow here
		{
			flag = 1;
			if (ec < ideal_graph[v_x][i].second) // need to substitute the length
			{
				ideal_graph[v_x][i].second = ec;
				midnode_graph[v_x][i].left = l;
				midnode_graph[v_x][i].right = r;
			}
			break;
		}
	if (!flag) // otherwise add an new edge
	{
		ideal_graph[v_x].push_back({ v_y, ec });
		midnode_graph[v_x].push_back({ l,r });
	}
}

void remove(graph_v_of_v_idealID& ideal_graph, int v_x, int v_y, double ec, vector<vector<struct midnode>>& midnode_graph)
{
	pair<int, double> p1(v_y, ec);
	vector<pair<int, double>> ::iterator  it = find(ideal_graph[v_x].begin(), ideal_graph[v_x].end(), p1);
	if (it != ideal_graph[v_x].end())
	{
		midnode_graph[v_x].erase(it - ideal_graph[v_x].begin() + midnode_graph[v_x].begin());
		ideal_graph[v_x].erase(it);
	}
}
	...
	*/
	...
	for (int i = 1; i <= N; i++)  // repeat
	{
		...
		while (1)
		{
			nd = q.top();
			q.pop();
			if (!isIntree[nd.vertex] && ideal_graph[nd.vertex].size() == nd.degree) break; // nd.vertex is the lowest degree vertex not in tree
		}
	...
	for (int i = bound_lambda + 1; i <= N; i++)  // take advantage of the priority queue
	{
		...
		while (1)
		{
			nd = q.top();
			q.pop();
			if (!isIntree[nd.vertex] && !popped_isIncore_node[nd.vertex] && ideal_graph[nd.vertex].size() == nd.degree) break;
		}
