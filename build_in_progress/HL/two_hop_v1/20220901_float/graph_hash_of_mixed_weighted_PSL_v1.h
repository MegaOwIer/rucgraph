...
void graph_hash_of_mixed_weighted_PSL_v1_thread_function_dij(int u, graph_hash_of_mixed_weighted_two_hop_case_info_v1* case_info)
{
	...
	*/
	...
	for (int i = 0; i < u_adj_size; i++)        // 遍历u的邻居v
	{
		...
		float ec = ideal_graph_595[u][i].second;        //(u,v)距离
		int v_label_size = pos_595[v] + increment_595[v]; // label size in the last iteration
		...
		{		
			...
			for (int j = 0; j < v_adj_size; j++)
			{
				...
				float ec_2 = ideal_graph_595[v][j].second; // v到v1的边的长度
				//遍历v_1的d-2级别label,用上pos_2
				for (int k = pos_2_595[v_1]; k < pos_595[v_1]; k++)
				{
					int w = L_595[v_1][k].vertex;

					if (w > u)
						continue; // cannot be the hub of u

					/* query pruning */
					float dis = L_595[v_1][k].distance + ec + ec_2;
					int w_label_size = L_595[w].size();
					bool flag = false;
					for (int k = 0; k < w_label_size; k++)
						if (!dirty_tag_595[used_id][L_595[w][k].vertex])
						{
							float query_dis = T_dij_595[used_id][L_595[w][k].vertex] + L_595[w][k].distance;
		...
		{
			for (int j = pos_595[v]; j < v_label_size; j++)
			{
				int w = L_595[v][j].vertex; // visit each hub of v
				if (w > u)
					continue; // cannot be the hub of u

				/* query pruning */
				float dis = L_595[v][j].distance + ec;
				int w_label_size = L_595[w].size();
				bool flag = false;
				for (int k = 0; k < w_label_size; k++)
					if (!dirty_tag_595[used_id][L_595[w][k].vertex])
	...
	if (labal_size_595 > max_labal_size_595) {
		throw reach_limit_error_string;  // after catching error, must call terminate_procedures_595(), otherwise this PSL cannot be reused
	}
...
void graph_hash_of_mixed_weighted_PSL_v1_thread_function_add_new_labels(int u)
{
	...
	pos_595[u] += increment_595[u]; increment_595[u] = L_temp_595[u].size();
...
(graph_hash_of_mixed_weighted& input_graph, int max_N_ID, int num_of_threads, graph_hash_of_mixed_weighted_two_hop_case_info_v1& case_info) {
	...
	for (int v_k = 0; v_k < N; v_k++)  //initialization
	{
		...
		pos_595[v_k] = 0; // record the position of the begining of the label in last iteration
		pos_2_595[v_k] = 0;
		increment_595[v_k] = 1;
		two_hop_label_v1 xx;
		xx.vertex = v_k;
	...
	while (if_continue_595 || if_continue_595_false_time < 2) // since R2 skip some vertices, some new labels can only be generated when d increases 2, not 1, thus terminate the loop only when if_continue_595==false twice
	{
		...
		for (int u = 0; u < N; u++)
		{
			...
			results.emplace_back(
				...
				graph_hash_of_mixed_weighted_PSL_v1_thread_function_dij(u, xx);
				return 1; // return to results; the return type must be the same with results
			})
			);
			push_num++;
		...
		for (int u = 0; u < N; u++)
		{
			...
			results.emplace_back(
				...
				graph_hash_of_mixed_weighted_PSL_v1_thread_function_add_new_labels(u);
				return 1; // return to results; the return type must be the same with results
			})
			);
		}
	...
	for (int u = 0; u < N; u++)
	{
		...
		for (int i = 0; i < u_label_size; i++)
		{
			int w = L_595[u][i].vertex; float dis = L_595[u][i].distance;
			if (dirty_tag_595[0][w]) // the same hub may have redundancy, record the shortest distance
			{
				dirty_tag_595[0][w] = false;
				T_dij_595[0][w] = dis;
				pre[w] = L_595[u][i].parent_vertex;
			}
			else if (dis < T_dij_595[0][w])
			{
				T_dij_595[0][w] = dis;
				pre[w] = L_595[u][i].parent_vertex;
			}
		...
		{
			int w = L_595[u][i].vertex;
			if (!dirty_tag_595[0][w])
			{
				dirty_tag_595[0][w] = true;
				two_hop_label_v1 xx;
				xx.vertex = w;
				xx.distance = T_dij_595[0][w];
