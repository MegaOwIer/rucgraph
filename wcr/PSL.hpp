...
class PSL {
    ...
    size_t build_PSL_labels(int num_of_threads);
...
PSL<weight_t>::PSL(const dgraph<weight_t> &g, int num_of_threads, runtime_info &cfg) {
    ...
    // build labels
    cfg.label_size_before = build_PSL_labels(num_of_threads);
    cfg.label_size_after = get_label_set_size();
    ...
    cfg.time_total =
        std::chrono::duration_cast<std::chrono::nanoseconds>(global_end_time - global_begin_time)
            .count();
}
...
void PSL<weight_t>::propagate(int k, size_t u) {
    ...
    std::function<bool(size_t, weight_t)> is_redundant = [&](size_t x, weight_t d_temp) {
        for (size_t j = 0; j < L[k ^ 1][x].size(); j++) {
...
template <class weight_t>
size_t PSL<weight_t>::build_PSL_labels(int num_of_threads) {
    // Line 1-2
    for (int k = 0; k < 2; k++) {
        L[k].resize(G[k]->getV());
    ...
    size_t label_size_before = get_label_set_size();
...
template <class weight_t>
size_t PSL<weight_t>::get_label_set_size() {
    size_t ans = 0;
    for (int i = 0; i < 2; i++) {
        for(auto &cur : L[i]) {
            ans += cur.size();
