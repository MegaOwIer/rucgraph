...
typedef PairingHeapYS_min<double, int> PairingHeap1_211; // define data types in heap
#include <PairingHeapYS_with_offset.h>
typedef PairingHeapYS_with_offset<double, int> PairingHeap2_211; // define data types in heap


class FGW_cluster_info_FGWgraph_v_of_v_idealID
{
public:
	bool activity = false; // false means inactive
	double event_time = 0; // the event time for each C
	PairingHeap2_211 C_ep_PQ;
	PairingHeap1_211::ItemHandle handle_Cevent;
	PairingHeap1_211::ItemHandle handle_Eevent;
};


graph_hash_of_mixed_weighted graph_v_of_v_idealID_Fast_GW_Growing_tree(graph_v_of_v_idealID& input_graph, vector<double>& vertex_weights, string return_content) {

	if (return_content != "tree" && return_content != "forest") {
		std::cout << "Error in graph_v_of_v_idealID_Fast_GW_Growing_idealvertexID: " <<
	...
	long long int ep_num = graph_v_of_v_idealID_total_edge_num(input_graph) * 2; // total number of edge parts
	...
	*/
	vector<FGW_cluster_info_FGWgraph_v_of_v_idealID> Clusters(N);
	vector<int> Union_Find_parent(N), Union_Find_setsize(N);
	for (int i = 0; i < N; i++) {
		FGW_cluster_info_FGWgraph_v_of_v_idealID x;
		Clusters[i] = x;
		Union_Find_parent[i] = i;
		Union_Find_setsize[i] = 1;
	}
	int sets_num = N;

	/*edge parts: PQ and their handles; time complexity O(|E|)*/
	std::vector<PairingHeap2_211::ItemHandle> handle_ep(ep_num); // store the handle for each edge part
	vector<int> ep_v1_list(ep_num);
	vector<int> ep_v2_list(ep_num);
	vector<double> ep_EventTime_list(ep_num, 0);
	PairingHeap1_211 C_event_PQ, E_event_PQ;
	...
	for (int i = 0; i < N; i++) {
		...
		for (int adj = 0; adj < v_size; adj++) {
			int j = input_graph[i][adj].first; // the adjacent vetex to i
			if (j > i) { // don't overcheck an edge
				// the first ep
				ep_v1_list[ep_order] = i;
				ep_v2_list[ep_order] = j;
				ep_EventTime_list[ep_order] = input_graph[i][adj].second / distribution_ratio; // halve the edge cost
	...
	while (Active_C_num > 1) // stop the loop when there is only 1 active clusters left
	{
		...
		if (Tc >= Te) {
			...
			C2 = Union_Find_find(ep_v2_list[ep1], Union_Find_parent); // the cluster C2 for this edge event
			...
			if (C1 == C2) { /*the inside ep is triggered; C_ep_PQ[C1].size at most V; time complexity: O(log V)*/
				...
				if (Clusters[C2].activity == true) { // C2 is active; time complexity: O(logV)
					...
					if (r > tiny_value) { /*r is big; time complexity: O(logV)*/
						...
						if (Union_Find_setsize[Union_Find_find(C1, Union_Find_parent)]
							< Union_Find_setsize[Union_Find_find(C2, Union_Find_parent)]) {
						...
						Union_Find_Union(C1, C2, Union_Find_parent, Union_Find_setsize, sets_num);
	...
	if (return_content == "tree") {
		...
		for (int i = 0; i < N; i++) {
			if (Clusters[i].activity == true) {
				double nw1 = vertex_weights[i];
		...
		for (int i = 0; i < merged_edges.size(); i++) {
			if (Clusters[Union_Find_find(merged_edges[i].first, Union_Find_parent)].activity == true) { // an edge in active cluster
				int v1 = merged_edges[i].first;
				int v2 = merged_edges[i].second;
				double nw1 = vertex_weights[v1];
				double nw2 = vertex_weights[v2];
	...
	for (int i = 0; i < N; i++) {
		delete(Clusters[i].handle_Cevent);
		delete(Clusters[i].handle_Eevent);
	}
