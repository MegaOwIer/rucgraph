...
public:
    ...
    auto enqueue(F &&f, Args &&...args) -> std::future<
        typename std::result_of<F(Args...)>::type>;  // 将任务添加到线程池的任务队列中
    ~ThreadPool();                                   // 线程池的析构函数
private:
    ...
    std::queue<std::function<void()>>
        tasks;  // 用于存放任务的队列，用queue队列进行保存。任务类型为std::function<void()>。因为std::function是通用多态函数封装器，本质上任务队列中存放的是一个个函数

    // synchronization
    std::mutex
        queue_mutex;  // 一个访问任务队列的互斥锁，在插入任务或者线程取出任务都需要借助互斥锁进行安全访问
    std::condition_variable
        condition;  // 一个用于通知线程任务队列状态的条件变量，若有任务则通知线程可以执行，否则进入wait状态
    bool stop;  // 标识线程池的状态，用于构造与析构中对线程池状态的了解
};

// the constructor just launches some amount of workers
inline ThreadPool::ThreadPool(
    size_t threads)  // 构造函数定义为inline。接收参数threads表示线程池中要创建多少个线程。
    : stop(false)  // stop初始为false，即表示线程池启动着。
{
    for (size_t i = 0; i < threads;
         ++i)  // 进入for循环，依次创建threads个线程，并放入线程数组workers中。
        workers.emplace_back(
            ...
            所以下述lambda表达式为 [ 捕获 ] { 函数体 }
            类型。传入的lambda表达式就是创建线程的执行函数.
            */
                ...
                for (;;) {
...

                    {
                        ...
                        */
                        ...
                        在退出了{
                        }，我们队任务队列的所加的锁也释放了，然后我们的线程就可以执行我们拿到的任务task了，执行完毕之后，线程又进入了死循环。
                        */
...
auto用于自动推导出equeue的返回类型，函数的形参为(F&& f, Args&&...
args)，其中&&表示右值引用。表示接受一个F类型的f，与若干个Args类型的args。
...
auto ThreadPool::enqueue(F &&f, Args &&...args) -> std::future<
    typename std::result_of<F(Args...)>::type>  // 表示返回类型，与lambda表达式中的表示方法一样。
{
    using return_type =
        typename std::result_of<F(Args...)>::type;  // 获得以Args为参数的F的函数类型的返回类型
    ...
    std::future<return_type> res =
        task->get_future();  // res中保存了类型为return_type的变量，有task异步执行完毕才可以将值保存进去
    {
...
保存多线程执行结果 for (int i = 0; i < 10; ++i) { // 创建10个任务 int j = i + 10;
        results.emplace_back(  // 保存每个异步结果
            pool.enqueue([j] { // 将每个任务插入到任务队列中，lambda表达式： pass const type value j
...
get后results里面的future就不能用了；clear之后results才能被再次使用 std::cout << std::endl;
