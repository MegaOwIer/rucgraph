...
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_Fast_GW_Growing_tree(graph_hash_of_mixed_weighted& input_graph, string return_content) {
	...
	*/
	...
	for (auto it = input_graph.hash_of_vectors.begin(); it != input_graph.hash_of_vectors.end(); it++) {
		FGW_cluster_info x;
		x.handle_Cevent = NULL; // pointers to NULL initially, for deleting pointers in the end
		x.handle_Eevent = NULL;
		Clusters[it->first] = x;
	...
	for (auto it = input_graph.hash_of_vectors.begin(); it != input_graph.hash_of_vectors.end(); it++) {
		...
		if (search != input_graph.hash_of_hashs.end()) {
			for (auto it2 = search->second.begin(); it2 != search->second.end(); it2++) {
				...
				// the first ep
					...
					ep_EventTime_list[ep_order] = it2->second
		...
		/*for active cluster*/
		double nw_i = it->second.vertex_weight;
		if (nw_i > 0) {
	...
	if (Active_C_num == 0) {
		graph_hash_of_mixed_weighted_add_vertex(solution_tree, input_graph.hash_of_vectors.begin()->first, input_graph.hash_of_vectors.begin()->second.vertex_weight);
		return solution_tree; // all vertex weights are 0, return the first single vertex
	}
	...
	for (auto it = Clusters.begin(); it != Clusters.end(); it++) {
		if (it->second.handle_Cevent != NULL) // pointers to NULL initially, see Lines 82-91
			delete(it->second.handle_Cevent);
		if (it->second.handle_Eevent != NULL)
