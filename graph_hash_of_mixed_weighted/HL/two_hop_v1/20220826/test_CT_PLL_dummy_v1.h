...
#include <graph_hash_of_mixed_weighted/HL/two_hop_v1/test_CT_PLL_dummy_v1.h>
...
pair<double, double> querying_element(graph_hash_of_mixed_weighted_CT_v1_case_info* case_info, int s, int t) {
...
pair<double, double> querying(graph_hash_of_mixed_weighted_CT_v1_case_info& case_info, vector<pair<int, int>>& query_list) {
	...
	graph_hash_of_mixed_weighted_CT_v1_case_info* case_info_p = &case_info;
...
void test_CT_PLL_dummy() {
	...
	int max_non_dummy_ID = 50;

	double avg_CT_time = 0, avg_CT_dummy_time = 0, avg_PLL_time = 0, avg_PLL_dummy_time = 0;
	long long int avg_CT_index_bit_size = 0, avg_CT_dummy_index_bit_size = 0, avg_PLL_index_bit_size = 0, avg_PLL_dummy_index_bit_size = 0;
	...
	for (int i = 0; i < iteration_graph_times; i++) {
		...
		for (int v = max_non_dummy_ID + 1; v < V; v++) {
			std::vector<int> adj_v1 = instance_graph.adj_v(v);
			for (int i = 0; i < adj_v1.size(); i++) {
				int v2 = adj_v1[i];
				new_ec.push_back({ {v, v2} ,1e6 });
			}
		}
		int size = new_ec.size();
		for (int i = 0; i < size; i++) {
			graph_hash_of_mixed_weighted_add_edge(instance_graph, new_ec[i].first.first, new_ec[i].first.second, new_ec[i].second);
		}
		...
		if (1) {
			...
			auto end = std::chrono::high_resolution_clock::now();
			double runningtime = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / 1e9; // s
			avg_CT_dummy_time = avg_CT_dummy_time + runningtime / iteration_graph_times;
			avg_CT_dummy_index_bit_size = avg_CT_dummy_index_bit_size + case_info.compute_label_bit_size() / iteration_graph_times;
		...
		//graph_hash_of_mixed_weighted_shortest_paths_source_to_all(instance_graph, 67, distances, predecessors);
		//cout << "distances[30] " << distances[30] << endl;
		//cout << "distances[42] " << distances[42] << endl;
		//cout << "predecessors[30] " << predecessors[30] << endl;
		//cout << "predecessors[42] " << predecessors[42] << endl;
		//cout << "predecessors[43] " << predecessors[43] << endl;
		...
		//cout << "max_non_dummy_ID + 1=" << max_non_dummy_ID + 1 << endl;

		/*query*/
		if (1) {
			vector<pair<int, int>> query_list;
			boost::random::uniform_int_distribution<> dist0{ static_cast<int>(0), static_cast<int>(max_non_dummy_ID) };
			boost::random::uniform_int_distribution<> dist1{ static_cast<int>(max_non_dummy_ID + 1), static_cast<int>(V - 1) };
			for (int i = 0; i < 1e3; i++) {
				int source = dist0(gen);
				int terminal = dist1(gen);
		...
		//boost::random::uniform_int_distribution<> dist0{ static_cast<int>(max_non_dummy_ID + 1), static_cast<int>(V - 1) };
		...
		if (0) {
			...
			mm.use_enhanced2019R2 = case_info.two_hop_case_info.use_enhanced2019R2;
			mm.use_non_adj_reduc_degree = case_info.two_hop_case_info.use_non_adj_reduc_degree;
			mm.use_dummy_dij_search_in_PLL = 1;
			auto begin = std::chrono::high_resolution_clock::now();
			graph_hash_of_mixed_weighted_PLL_v1(instance_graph, V + 1, 1, case_info.thread_num, mm);
			auto end = std::chrono::high_resolution_clock::now();
			double runningtime = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count() / 1e9; // s
			avg_PLL_dummy_time = avg_PLL_dummy_time + runningtime / iteration_graph_times;
			avg_PLL_dummy_index_bit_size = avg_PLL_dummy_index_bit_size + mm.compute_label_bit_size() / iteration_graph_times;
		}
	}
	...
	cout << "avg_CT_dummy_time: " << avg_CT_dummy_time << "s" << endl;
	cout << "avg_PLL_time: " << avg_PLL_time << "s" << endl;
	cout << "avg_PLL_dummy_time: " << avg_PLL_dummy_time << "s" << endl;
	cout << "avg_CT_index_bit_size: " << avg_CT_index_bit_size << endl;
	cout << "avg_CT_dummy_index_bit_size: " << avg_CT_dummy_index_bit_size << endl;
	cout << "avg_PLL_index_bit_size: " << avg_PLL_index_bit_size << endl;
	cout << "avg_PLL_dummy_index_bit_size: " << avg_PLL_dummy_index_bit_size << endl;
}