...
an example cpp file for running the test function in the end of this file (make sure the Boost and ysgraph folders are loaded):
--------------------------------------------------------------------------------------
#include <iostream>
using namespace std;
...
int main()
{
	test_graph_hash_of_mixed_weighted_DPBF_edge_weighted();
}
-----------------------------------------------------------------------------------------------------
*/
...
#pragma region
int graph_hash_of_mixed_weighted_DPBF_edge_weighted_vertex_group_set_ID(int vertex, graph_hash_of_mixed_weighted& group_graph,
...
#pragma endregion graph_hash_of_mixed_weighted_DPBF_edge_weighted_vertex_group_set_ID

#pragma region
struct graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node {
	int v;
	int p; // group_set_ID
	double priority_value; // cost of T(v,p)
};
bool operator<(graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node const& x, graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node const& y) {
...
typedef typename boost::heap::fibonacci_heap<graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node>::handle_type handle_graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node;
#pragma endregion graph_hash_of_mixed_weighted_DPBF_edge_weighted priority queue

#pragma region
class graph_hash_of_mixed_weighted_DPBF_edge_weighted_tree_node
{
...
#pragma endregion graph_hash_of_mixed_weighted_DPBF_edge_weighted_tree_node

#pragma region
std::unordered_map<int, std::unordered_set<int>> graph_hash_of_mixed_weighted_DPBF_edge_weighted_non_overlapped_group_sets(int group_sets_ID_range) {
...
#pragma endregion graph_hash_of_mixed_weighted_DPBF_edge_weighted_non_overlapped_group_sets

#pragma region
graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_DPBF_edge_weighted_build_tree(int root_v, int root_p, graph_hash_of_mixed_weighted& input_graph,
	std::unordered_map<int, std::unordered_map<int, graph_hash_of_mixed_weighted_DPBF_edge_weighted_tree_node>>& trees) {
...
#pragma endregion graph_hash_of_mixed_weighted_DPBF_edge_weighted_build_tree

graph_hash_of_mixed_weighted graph_hash_of_mixed_weighted_DPBF_edge_weighted(
	...
	/*Group		G1	G0	group_set_ID
				...
				1	0	2
				1	1	3*/
	std::unordered_map<int, std::unordered_set<int>> non_overlapped_group_sets_IDs = graph_hash_of_mixed_weighted_DPBF_edge_weighted_non_overlapped_group_sets(group_sets_ID_range);  // time complexity: O(4^|Gamma|)
	boost::heap::fibonacci_heap<graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node> Q_T; // min queues of trees
	std::unordered_map<string, handle_graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node> Q_T_handles; // key is string "v_p" ("vertex_ID" + "_" + "group set ID"); Q_T_handles only contain keys that are in Q_T
	...
	*/
	std::unordered_map<int, std::unordered_map<int, graph_hash_of_mixed_weighted_DPBF_edge_weighted_tree_node>> trees; // <v, <p, T(v,p)>>
	for (auto it = input_graph.hash_of_vectors.begin(); it != input_graph.hash_of_vectors.end(); it++) {
		int v = it->first; // a vertex
		trees[v] = {};
		int group_set_ID_v = graph_hash_of_mixed_weighted_DPBF_edge_weighted_vertex_group_set_ID(v, group_graph, cumpulsory_group_vertices); /*time complexity: O(|Gamma|)*/
		for (int p = 1; p <= group_set_ID_v; p++) { // p is non-empty; time complexity: O(2^|Gamma|)
			if ((p | group_set_ID_v) == group_set_ID_v) { // p represents a non-empty group set inside group_set_ID_v, including group_set_ID_v
				...
				/*T(v,p)*/
				graph_hash_of_mixed_weighted_DPBF_edge_weighted_tree_node node;
				node.cost = 0;
				node.type = 0;
				trees[v][p] = node;

				/*insert T(v,p) into Q_T*/
				graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node x;
				x.v = v;
				x.p = p;
				x.priority_value = 0;
	...
	while (Q_T.size() > 0) { // at most 2^|Gamma|*V loops
		...
		graph_hash_of_mixed_weighted_DPBF_edge_weighted_min_node top_node = Q_T.top();
		...
		if (p == group_sets_ID_range) { // T(v,p) covers all groups 						
			return graph_hash_of_mixed_weighted_DPBF_edge_weighted_build_tree(v, p, input_graph, trees); // time complexity: O(|V|)
		}
	...
	std::cout << "graph_hash_of_mixed_weighted_DPBF_edge_weighted did not find a feasible solution!" << std::endl;
	getchar();
	exit(1);
...
#pragma region
void test_graph_hash_of_mixed_weighted_DPBF_edge_weighted() {
	...
	double solution_cost_DPBF_sum = 0;
	double time_DPBF_avg = 0;
...
#pragma endregion test_graph_hash_of_mixed_weighted_DPBF_edge_weighted